...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    if (!verify_csrf()) {
        ...
        redirect('admin/lecturers/create.php');
    }
    ...
    if ($errors === []) {
        try {
            create_lecturer($form);
            set_flash('success', 'Lecturer created successfully.');
            redirect('admin/lecturers/index.php');
        } catch (InvalidArgumentException $exception) {
            $errors[] = $exception->getMessage();
        } catch (Throwable $exception) {
            error_log('Lecturer creation failed: ' . $exception->getMessage());
            $errors[] = 'Unable to create the lecturer.';
        }
    }
}
